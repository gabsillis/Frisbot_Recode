...
public class LEDString extends Subsystem {
	...
	public LEDString() {
		try {
			...
			System.err.println("I2C Communication initialization error");
			e.printStackTrace();
			initialized = false;
		}
	}
		...
		if (initialized) {
			try {
				...
				if (Robot.shooter.isActive())
					data[SHOOTER_ACTIVE_INDEX] = 1;
				else
					data[SHOOTER_ACTIVE_INDEX] = 0;
				data[SHOOTER_SPEED_INDEX] = ((byte) ((Robot.shooter.getSpeed() * 256) - 127));
				if (Robot.shooter.isPistonExtended())
					data[SHOOTING_INDEX] = 1;
				else
					data[SHOOTING_INDEX] = 0;
				if (OI.driveStick.getRawButton(4)
						&& OI.driveStick.getRawButton(3)
						&& OI.driveStick.getRawButton(5))
					data[EASTER_EGG_INDEX] = 1;
				else
					data[EASTER_EGG_INDEX] = 0;
				i2c.writeBulk(data);
			} catch (Exception e) {
				System.err.println("I2C Communication Error");
			}
		}
	}
}
